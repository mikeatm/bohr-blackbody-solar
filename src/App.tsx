...
import AtomicTrendsApp from "./components/AtomicTrendsApp";
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 p-4">
      ...
      <div className="flex justify-center gap-4 mb-6">
        {['bohr','blackbody','spectrum','periodic'].map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg ${tab===t ? 'bg-blue-600' : 'bg-gray-700 hover:bg-gray-600'}`}
          >
            {t === 'bohr' ? 'Bohr Model' : t === 'blackbody' ? 'Blackbody Radiation' : t === 'spectrum' ? 'Solar Spectrum' : 'Periodic Table'}
          </button>
        ))}
      </div>
